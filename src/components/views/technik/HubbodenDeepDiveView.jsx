...
const HOTSPOTS = [
  ...
  { id: 'hydraulik', label: 'Hydraulikzylinder', short: 'HYD', color: '#a070ff', x: 250, y: 330 },
...
const HOTSPOT_DATA = {
  ...
  hydraulik: { title: 'Hydraulikzylinder', color: '#a070ff', items: ['Anzahl: 4–8 Zylinder je nach Beckengröße', 'Antrieb: Hydrauliköl-Druckanlage', 'Hubgeschwindigkeit: ca. 0,3 m/min (sicherheitsbedingt)', 'Synchronsteuerung: alle Zylinder gleichzeitig', 'Notabsenkung: bei Stromausfall automatisch', 'Drucküberwachung: kontinuierlich, Alarm bei Abweichung'] },
  ...
  sicherung: { title: 'Sicherheitseinrichtungen', color: '#ffd166', items: ['Lichtschranken oder Ultraschall: Personen im Becken', 'NOT-HALT-Taster: am Beckenrand + am Bedienpanel', 'Absicherung Wasserlinie: Schwimmer darf nicht eingeklemmt werden', 'Mechanische Endanschläge: verhindert Überfahren', 'Optisch-akustische Warnung: vor jeder Bewegung', 'UVV: Becken muss bei Betrieb gesperrt sein'] },
  ...
  wartung: { title: 'Wartung & Prüfung', color: '#5ad0ff', items: ['Tägliche Funktionsprüfung vor Öffnung (Protokoll)', 'Hydraulikflüssigkeit: jährlicher Wechsel', 'Zylinderdichtungen: Inspektion alle 2 Jahre', 'Prüfung nach DGUV V70 + Herstellervorgaben', 'Einweisung des Bedienpersonals: dokumentiert', 'Kein Betrieb durch uneingewiesene Personen'] },
};

const MODES = [
  { id: 'aufbau', label: 'Aufbau & Technik', color: '#4a9eff', focus: ['bodenplatte', 'hydraulik', 'abdichtung'] },
...
const KENNDATEN = [
  ...
  { label: 'Antrieb', value: 'Hydraulisch (4–8 Zylinder)' },
  { label: 'Hubgeschwindigkeit', value: 'ca. 0,3 m/min (Sicherheitsvorschrift)' },
  { label: 'Steuerung', value: 'SPS mit Schlüsselschalter, NOT-HALT' },
  ...
  return (
    <div style={{ width: '100%', height: 'clamp(320px, 55vh, 520px)' }}>
      <svg viewBox="0 0 820 420" style={{ width: '100%', height: '100%' }}>
        ...
        {/* Beckenwände */}
        <rect x="80" y="100" width="480" height="300" rx="3" fill="#061520" stroke="#1a3a5a" strokeWidth="3" />
        {/* Wasser */}
        <rect x="83" y="103" width="474" height="197" rx="2" fill="url(#hb-water)" />
        <text x="320" y="200" textAnchor="middle" fill="#1a5a8a" fontSize="13" fontWeight="700">WASSER</text>
        <text x="320" y="216" textAnchor="middle" fill="#0e3a5a" fontSize="10">variabel je nach Hubposition</text>

        {/* Hubboden Platte */}
        <rect x="83" y="298" width="474" height="22" rx="2" fill="#0d2040" stroke="#4a9eff" strokeWidth="2.5" />
        <text x="320" y="313" textAnchor="middle" fill="#4a9eff" fontSize="10" fontWeight="700">HUBBODEN-PLATTE (Stahlbeton)</text>

        {/* Hydraulikzylinder */}
        {[140, 230, 390, 480].map((x, i) => (
          <g key={i}>
            <rect x={x - 10} y={320} width="20" height="70" rx="3" fill="#1a0a3a" stroke="#a070ff" strokeWidth="1.5" />
            <rect x={x - 5} y={314} width="10" height="10" rx="2" fill="#a070ff" opacity="0.7" />
            <line x1={x} y1="390" x2={x} y2="395" stroke="#a070ff" strokeWidth="2" />
          </g>
        ))}
        <text x="170" y="408" textAnchor="middle" fill="#7040c0" fontSize="9">Hydraulikzylinder</text>
        <text x="430" y="408" textAnchor="middle" fill="#7040c0" fontSize="9">synchron gesteuert</text>

        {/* Dichtlippe Wand */}
        <rect x="77" y="290" width="10" height="30" rx="2" fill="#2a0a0a" stroke="#ff7a7a" strokeWidth="1.5" />
        <rect x="553" y="290" width="10" height="30" rx="2" fill="#2a0a0a" stroke="#ff7a7a" strokeWidth="1.5" />
        <text x="70" y="288" textAnchor="end" fill="#ff7a7a" fontSize="8">Dichtlippe</text>

        {/* Wasserstand Marker */}
        <line x1="580" y1="103" x2="580" y2="300" stroke="#4a9eff" strokeWidth="1" strokeDasharray="3,4" opacity="0.4" />
        <text x="592" y="108" fill="#4a9eff" fontSize="9">WO</text>
        <text x="592" y="304" fill="#4a9eff" fontSize="9">HB</text>
        <line x1="578" y1="103" x2="590" y2="103" stroke="#4a9eff" strokeWidth="1.5" />
        <line x1="578" y1="300" x2="590" y2="300" stroke="#4a9eff" strokeWidth="1.5" />
        <text x="635" y="205" textAnchor="middle" fill="#4a9eff" fontSize="11" fontWeight="700">Hubhöhe</text>
        <text x="635" y="220" textAnchor="middle" fill="#4a9eff" fontSize="11">≈ 2,00 m</text>

        {/* Steuereinheit */}
        <rect x="620" y="100" width="150" height="90" rx="6" fill="#001a10" stroke="#34c09050" strokeWidth="1.5" />
        <text x="695" y="120" textAnchor="middle" fill="#34c090" fontSize="10" fontWeight="700">Steuereinheit SPS</text>
        <rect x="635" y="128" width="30" height="18" rx="3" fill="#0a3020" stroke="#34c090" strokeWidth="1" />
        <text x="650" y="141" textAnchor="middle" fill="#34c090" fontSize="8">KEY</text>
        <rect x="675" y="128" width="18" height="18" rx="9" fill="#ff3030" stroke="#ff6060" strokeWidth="1.5" />
        <text x="684" y="141" textAnchor="middle" fill="#fff" fontSize="7" fontWeight="700">!</text>
        <text x="695" y="168" textAnchor="middle" fill="#1a7050" fontSize="9">Schlüsselschalter</text>
        <text x="695" y="180" textAnchor="middle" fill="#1a7050" fontSize="9">NOT-HALT</text>
  ...
  return (
    <div style={{ background: '#04111f', minHeight: '100vh', padding: '20px 16px', fontFamily: 'sans-serif' }}>
      <div style={{ maxWidth: 820, margin: '0 auto' }}>
        ...
        <p style={{ color: '#4a7a9a', fontSize: 13, marginBottom: 16 }}>Hydraulisch verstellbarer Beckenboden — Aufbau, Steuerung, Sicherheit und Wartung</p>
        ...
        <div style={{ background: '#071828', border: '1px solid #163651', borderRadius: 12, padding: 14 }}>
          ...
          {KENNDATEN.map(({ label, value }, i) => (
            <div key={i} style={{ display: 'flex', gap: 12, marginBottom: 6, fontSize: 13 }}>
              <span style={{ color: '#4a9eff', fontWeight: 700, minWidth: 180 }}>{label}</span>
