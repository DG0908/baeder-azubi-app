              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg font-medium transition-all ${berichtsheftViewMode === 'kalender' ? 'bg-cyan-500 text-white' : (darkMode ? 'bg-slate-700 text-gray-300' : 'bg-gray-200 text-gray-700')}`}
                  >
                    📅 Kalender
                  </button>
                  {canManageBerichtsheftSignatures && (
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <button
                        ...
                        className={`p-2 rounded-lg transition-all ${darkMode ? 'hover:bg-slate-600 text-gray-300' : 'hover:bg-gray-100 text-gray-600'}`}
                      >‹</button>
                    ...
                    <div className="flex gap-4 mb-3 text-xs">
                      <div className="flex items-center gap-1"><div className="w-2.5 h-2.5 rounded-full bg-green-400" /><span className={darkMode ? 'text-gray-300' : 'text-gray-600'}>Unterschrieben</span></div>
                      <div className="flex items-center gap-1"><div className="w-2.5 h-2.5 rounded-full bg-yellow-400" /><span className={darkMode ? 'text-gray-300' : 'text-gray-600'}>Azubi unterschrieben</span></div>
                      <div className="flex items-center gap-1"><div className="w-2.5 h-2.5 rounded-full bg-orange-400" /><span className={darkMode ? 'text-gray-300' : 'text-gray-600'}>Eingetragen</span></div>
                    </div>
                    {/* Day header */}
                    <div className="grid grid-cols-7 gap-1 mb-1">
                      {['Mo','Di','Mi','Do','Fr','Sa','So'].map(d => (
                        <div key={d} className={`text-center text-xs font-bold py-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>{d}</div>
                      ))}
                    </div>
                    {/* Calendar grid */}
                    <div className="grid grid-cols-7 gap-1">
                      {calDays.map((day, i) => {
                        ...
                        const dotColor = !entry ? null
                          : isSignedByAusbilder(entry) ? 'bg-green-400'
                          : isSignedByAzubi(entry) ? 'bg-yellow-400'
                          : 'bg-orange-400';
                        ...
                        return (
                          <button
                            ...
                            className={`flex flex-col items-center py-1.5 px-1 rounded-lg transition-all text-xs ${
                              isToday
                                ? (darkMode ? 'ring-2 ring-cyan-400 bg-cyan-900/30' : 'ring-2 ring-cyan-500 bg-cyan-50')
                                : entry
                                  ? (darkMode ? 'hover:bg-slate-600 cursor-pointer' : 'hover:bg-gray-100 cursor-pointer')
                                  : 'cursor-default'
                            } ${!isCurrentMonth ? 'opacity-30' : ''}`}
                          >
                            <span className={`font-medium ${darkMode ? (isCurrentMonth ? 'text-white' : 'text-gray-600') : (isCurrentMonth ? 'text-gray-800' : 'text-gray-300')}`}>
                              {day.getDate()}
                            </span>
                            {dotColor && (
